...
@section('content')
      <div class="pt-20 pb-4 px-9">
            <div class="w-full xl:pl-64 lg:pl-52">
                  <form action="/dashboard/blog" method="post" enctype="multipart/form-data" class="flex flex-col w-full mx-auto lg:w-full md:2/3">
                        @csrf
                        <h1 class="mb-5 text-lg font-semibold text-center text-hitam lg:text-2xl md:text-xl">Create New Blog</h1>
                        {{-- title --}}
                        <label class="mb-1 text-sm font-medium text-hitam lg:text-md" for="title">Title</label>
                        <input type="text" name="title" class="@error('title')border-red-500 border-2  @enderror py-2 mb-3 px-4 bg-slate-100 rounded-md" value="{{ old('title') }}" id="title" placeholder="Title" required>
                        @error('title')
