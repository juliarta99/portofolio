...
@section('content')
      <div class="pt-20 pb-4 px-9">
            <div class="w-full xl:pl-64 lg:pl-52">
                  <a href="/dashboard/project/create" class="flex justify-center w-1/2 px-3 py-2 mb-3 font-semibold bg-blue-500 rounded-md lg:w-1/4 xl:w-1/6 text-hitam">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <h3 class="ml-2">Project Baru</h3>
                  </a>
                  @if (session()->has('succes'))
                  <div class="flex w-1/2 p-4 mb-5 bg-green-400 rounded-md md:w-1/3 lg:w-1/4">
                        <p>{{ session('succes') }}</p>
                        <button>
                              <a href="/dashboard/project/deleteSession">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  ...
                  <table class="w-full text-center rounded-md shadow-md" id="table-dashboard">
                        ...
                        <tr class="text-sm even:bg-slate-200 bg-slate-100 xl:text-md">
                              <td class="p-4">{{ $project->id }}</td>
                              <td class="p-4">{{ $project->name }}</td>
                              <td class="flex flex-wrap items-center justify-center p-4">
                                    <a href="/dashboard/project/pemrograman/insert/{{ $project->id }}">
                                          <button class="py-1 px-2 m-0.5 lg:my-0 rounded-md bg-green-500">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
